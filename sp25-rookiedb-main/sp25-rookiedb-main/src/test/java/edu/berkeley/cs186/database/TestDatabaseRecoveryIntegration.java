...
public class TestDatabaseRecoveryIntegration {
    ...
    private static final Logger logger = LoggerFactory.getLogger(TestDatabaseRecoveryIntegration.class);
    ...
    private void reloadDatabase(boolean closeOld) {
        ...
        // Step 1: Close old database if needed
        long closeStartTime = System.currentTimeMillis();
        if (closeOld && this.db != null) {
            this.db.close();
        }
        if (closeOld) {
            logger.debug("Close old database took: {}ms", System.currentTimeMillis() - closeStartTime);
        }

        // Step 2: Clean up transaction context
        long cleanupStartTime = System.currentTimeMillis();
        if (TransactionContext.getTransaction() != null) {
            TransactionContext.unsetTransaction();
        }
        logger.debug("Transaction context cleanup took: {}ms", System.currentTimeMillis() - cleanupStartTime);

        // Step 3: Initialize lock manager
        long lockManagerStartTime = System.currentTimeMillis();
        this.lockManager = new LockManager();
        logger.debug("Lock manager initialization took: {}ms", System.currentTimeMillis() - lockManagerStartTime);

        // Step 4: Create new database instance
        long dbCreateStartTime = System.currentTimeMillis();
        this.db = new Database(this.filename, 128, this.lockManager, new ClockEvictionPolicy(), true);
        this.db.setWorkMem(32); // B=32
        logger.debug("Database creation took: {}ms", System.currentTimeMillis() - dbCreateStartTime);

        // Step 5: Load demo data if needed
        if (closeOld) {
            long loadDemoStartTime = System.currentTimeMillis();
            try {
                this.db.loadDemo();
                logger.debug("Load demo data took: {}ms", System.currentTimeMillis() - loadDemoStartTime);
            } catch (IOException e) {
                throw new DatabaseException("Failed to load demo tables.\n" + e.getMessage());
            }
        }

        // Step 6: Wait for all transactions to complete (recovery process)
        long waitStartTime = System.currentTimeMillis();
        this.db.waitAllTransactions();
        logger.debug("Wait all transactions (recovery) took: {}ms", System.currentTimeMillis() - waitStartTime);

        logger.debug("Total reloadDatabase took: {}ms", System.currentTimeMillis() - startTime);
    }
    ...
    public void testRebootCreateAndDropTable() {
        ...
        try(Transaction t1 = db.beginTransaction()) {
            for (int i = 0; i < 3; i++) {
                ...
                t1.createTable(new Schema().add("int", Type.intType()), "ints" + i);
                logger.debug("Create table ints{} took: {}ms", i, System.currentTimeMillis() - tableStartTime);

                long insertStartTime = System.currentTimeMillis();
                for (int j = 0; j < 1024 * 5; j++) {
                    t1.insert("ints"+i, j);
                }
                logger.debug("Insert 5120 records into ints{} took: {}ms", i, System.currentTimeMillis() - insertStartTime);
            }

            long dropStartTime = System.currentTimeMillis();
            t1.dropTable("ints0");
            logger.debug("Drop table ints0 took: {}ms", System.currentTimeMillis() - dropStartTime);
        }
        logger.debug("Transaction commit took: {}ms", System.currentTimeMillis() - startTime);

        Database old = this.db;
        long reloadStartTime = System.currentTimeMillis();
        reloadDatabase(false);
        logger.debug("Database reload took: {}ms", System.currentTimeMillis() - reloadStartTime);
        ...
        logger.debug("Query verification took: {}ms", System.currentTimeMillis() - queryStartTime);

        this.db.close();
        logger.debug("Total test time: {}ms", System.currentTimeMillis() - startTime);
    }
}
