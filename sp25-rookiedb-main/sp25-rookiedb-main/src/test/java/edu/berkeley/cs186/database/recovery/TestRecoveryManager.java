...
public class TestRecoveryManager {
    ...
    public void testEnd() {
        ...
        assertTrue(LogManager.maxLSN(LogManager.getLSNPage(LSNs[6])) < logManager.getFlushedLSN());
    }
    ...
    public void testAnalysisCheckpoint() {
        ...
        List<Long> LSNs = new ArrayList<>();
        //模拟事务操作 ：测试首先创建了一系列日志记录，模拟了多个事务的行为：
        //- 事务 1 (T1) ：更新了两个页面，然后正常结束（但它的结束记录在检查点开始之前）。
        //- 事务 2 (T2) ：更新了四个页面，并写入了 Commit 记录，使其进入 COMMITTING （正在提交）状态。
        //- 事务 3 (T3) ：写入了 Abort 记录，使其进入 ABORTING （正在中止）状态。
        //- 事务 4 (T4) ：没有任何提交或中止记录，因此处于 RUNNING （正在运行）状态。
        LSNs.add(logManager.appendToLog(new UpdatePageLogRecord(1L, 10000000001L, 0L, (short) 0, before, after))); // 0
