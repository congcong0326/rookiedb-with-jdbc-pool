...
public class Table implements BacktrackingIterable<Record> {
    ...
    /**
     * 数据页最前面bitmapSizeInBytes字节用于存储位图，已标识空闲的record
     * @param page
     * @return
     */
    ...
    private static int computeBitmapSizeInBytes(int pageSize, Schema schema) {
        ...
        // 字节对齐，8位为一个字节，recordsPerPage % 8 == 0为完美对齐
    ...
    public synchronized RecordId addRecord(Record record) {
        ...
        try {
            ...
            // 返回 page 页号与数据在 page 中的entryNum
    ...
    public synchronized Record deleteRecord(RecordId rid) {
        ...
        try {
            ...
            pageDirectory.updateFreeSpace(page, (short) ((numRecordsPerPage - numRecords) * schema.getSizeInBytes()));
            return record;
        } finally {
            page.unpin();
        }
