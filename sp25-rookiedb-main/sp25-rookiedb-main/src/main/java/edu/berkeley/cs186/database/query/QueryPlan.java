...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        // 顺序扫描操作符（SequentialScanOperator）作为默认的访问方法
        QueryOperator minOp = new SequentialScanOperator(this.transaction, table);

        // TODO(proj3_part2): implement
        int count = -8;
        int minIO = minOp.estimateIOCost();
        // 调用 getEligibleIndexColumns 方法获取表上所有可用的索引列。这个方法会返回 selectPredicates 列表中那些：
        //
        //- 属于当前表的谓词
        //- 该表上对应列存在索引
        //- 谓词操作符可用于索引扫描（不是 NOT_EQUALS）
        for (int index : getEligibleIndexColumns(table)) {
            SelectPredicate predicate = selectPredicates.get(index);
            //对每个可用索引，创建一个索引扫描操作符并估算其 IO 成本。索引扫描的 IO 成本计算考虑了：
            //
            //- B+ 树的高度
            //- B+ 树的阶数（order）
            //- 满足谓词条件的记录数量
            QueryOperator queryPlan = new IndexScanOperator(this.transaction, table, predicate.column, predicate.operator, predicate.value);
            int indexIO = queryPlan.estimateIOCost();
            //如果找到一个比当前最低成本更低的访问方法，就更新最低成本操作符和相关变量。
            if (minIO > indexIO) {
                count = index;
                minOp = queryPlan;
                minIO = indexIO;
        ...
        // 调用 addEligibleSelections 方法将其他适用的选择谓词应用到选定的访问方法上。
        // todo 当前并未考虑到联合索引的场景，只处理单列索引场景
        // 所以这里可以只需要比较全表扫描与单列索引的时间成本
        minOp = addEligibleSelections(minOp, count);
        return minOp;
    }
     ...
     * 假设有三张表 A, B, C，有如下连接条件：
     * A.a = B.a
     * B.b = C.b
     * 任务是找出 (A ⋈ B) ⋈ C、(B ⋈ C) ⋈ A、... 等所有可能的左深连接顺序中代价最低的一种。
     */
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        // prevMap 包含了前一轮已经连接的表集合及其最优操作符
        // pass1Map 包含了单表访问的最优方式（通常是索引扫描或顺序扫描）
        for (Set<String> key : prevMap.keySet()) {
            for (JoinPredicate predicate : this.joinPredicates) {
                QueryOperator table;
                Set<String> setKey = new HashSet<>(key);
                Set<String> set = new HashSet<>();
                QueryOperator tableJoin;
                if (!key.contains(predicate.leftTable) && key.contains(predicate.rightTable)) {
                    set.add(predicate.leftTable);
                    // 由于leftTable结果还未计算，获取leftTable的单表扫描成本
                    table = pass1Map.get(set);
                    // 得到 rightTable join  leftTable 的最小成本
                    tableJoin = minCostJoinType(prevMap.get(key), table, predicate.rightColumn, predicate.leftColumn);
                    setKey.add(predicate.leftTable);
                } else if (key.contains(predicate.leftTable) && !key.contains(predicate.rightTable)) {
                    set.add(predicate.rightTable);
                    table = pass1Map.get(set);
                    tableJoin = minCostJoinType(prevMap.get(key), table, predicate.leftColumn, predicate.rightColumn);
                    setKey.add(predicate.rightTable);
                } else {
                    continue;
                }
                if (result.containsKey(setKey)) {
                    if (result.get(setKey).estimateIOCost() > tableJoin.estimateIOCost())
                        result.put(setKey, tableJoin);
                } else {
                    result.put(setKey, tableJoin);
                }
            }
    ...
    public Iterator<Record> execute() {
        ...
        int runs = tableNames.size();
        // 构造每个单表的最小成本
        Map<Set<String>, QueryOperator> lowestCost = new HashMap<>();
        for (String table : tableNames) {
            Set<String> tableMap = new HashSet<>();
            tableMap.add(table);
            lowestCost.put(tableMap, minCostSingleAccess(table));
        }
        // 通过动态规划的思想，计算联表的最小成本
        Map<Set<String>, QueryOperator> joinTable = lowestCost;
        for (int i = 2; i <= runs; i++) {
            joinTable = minCostJoins(joinTable, lowestCost);
        }
        // 找到最小成本的执行计划
        finalOperator = minCostOperator(joinTable);
        addGroupBy();
        addProject();
        addLimit();
