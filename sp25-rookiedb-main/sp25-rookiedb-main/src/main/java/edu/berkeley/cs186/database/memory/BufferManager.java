...
public class BufferManager implements AutoCloseable {
    ...
    class Frame extends BufferFrame {
         ...
         * 使用一个index变量表达两种状态，完成一个逻辑上的链表结构
         * 负数代表空闲，并且指向下一个元素的索引的取反，正数代表使用
         * firstFreeIndex 为空闲链表的头部，链表中的节点通过index关联在一起
         * firstFreeIndex = 5
         *      ↓
         *    Frame[5] → Frame[3] → Frame[7] → null
         *    (index=-4) (index=-8) (index=0)
         *
         *    setFree代表将元素加入链表当中
         */
        ...
        /**
         * 从元素从链表中移除，firstFreeIndex 是头部元素的索引，需要将 firstFreeIndex 更新为下一个元素的索引
         */
    ...
    Frame fetchPageFrame(long pageNum) {
        ...
        try {
            ...
            if (this.firstFreeIndex < this.frames.length) {
                ...
                // setUsed会将空闲Frame移动出链表，更新链表头节点
                evictedFrame.setUsed();
            } else {
                // 空闲链表中每数据，通过LRU算法淘汰一个页面
