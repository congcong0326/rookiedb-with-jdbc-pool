...
public class Database implements AutoCloseable {
    ...
    public synchronized void waitAllTransactions() {
        logger.debug("Waiting for transactions to complete...");
        logger.debug("Active transactions count: {}", activeTransactions.getRegisteredParties());
        logger.debug("Arrived transactions count: {}", activeTransactions.getArrivedParties());
        logger.debug("Current phase: {}", activeTransactions.getPhase());
        logger.debug("Is terminated: {}", activeTransactions.isTerminated());


        while (!activeTransactions.isTerminated()) {
            logger.debug("Still waiting... Active: {}", activeTransactions.getRegisteredParties());
            activeTransactions.awaitAdvance(activeTransactions.getPhase());
        }
        logger.debug("All transactions completed!");
    }
    ...
    private Table tableFromMetadata(TableMetadata metadata) {
        ...
        // 从table元数据信息中获取表的page管理对象，表开始的pageNum为metadata.partNum的映射
    ...
    private class TransactionContextImpl extends TransactionContext {
        ...
        public void close() {
            try {
                ...
                // 创建一个副本，因为在释放过程中可能会修改原始列表
                List<Lock> locksCopy = new ArrayList<>(locks);
                locksCopy.sort((lock1, lock2) -> {
                    // 比较资源名称的深度，深度大的（叶子节点）排在前面
                    int depth1 = lock1.name.toString().split("/").length;
                    int depth2 = lock2.name.toString().split("/").length;
                    return Integer.compare(depth2, depth1); // 注意这里是降序排列
                });
                // 逐个释放所有锁
