 ...
 *
 * 每个header有一个bitmap，可以管理32K个page，master page 可以管理2k个header page，所以一个分区可以有 64M 个数据页，所以总容量为256GB
 ...
 * 分区文件结构（以4页/header为例）：
 * ┌─────┬─────┬─────┬─────┬─────┬─────┬─────┬─────┬─────┬─────┬─────┐
 * │  M  │  H  │  D  │  D  │  D ... D  │  H  │  D  │  D  │  D  │  D  │
 * └─────┴─────┴─────┴─────┴─────┴─────┴─────┴─────┴─────┴─────┴─────┘
 *    0     1     2     3     4 ...32K+1 32K+2 32K+3 32K+4 32K+5 32K+6
 *
 * M = Master Page (主页)
 * H = Header Page (头页)
 * D = Data Page (数据页)
 * - DATA_PAGES_PER_HEADER = PAGE_SIZE * 8 = 4096 * 8 = 32768 （32K个数据页/header）
 * - MAX_HEADER_PAGES = PAGE_SIZE / 2 = 2048 （最多2K个header页）
 * - 每个分区最多可以有 64M 个数据页（32K × 2K = 64M）
 * - 总容量可达 256GB（64M × 4KB = 256GB）
 */
