...
class LeafNode extends BPlusNode {
    ...
    public LeafNode getLeftmostLeaf() {
        // TODO(proj2): implement
        // 和 edu.berkeley.cs186.database.index.LeafNode.get 类似，同样返回自己
        return this;
    }
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        int i = InnerNode.numLessThan(key, keys);
        keys.add(i, key);
        rids.add(i, rid);
        if (keys.size() <= 2 * d) {
            sync();
        ...
        // 裂开
            ...
            // 生成新右子树
            // 新右子树的右兄弟为之前节点的右兄弟，所以直接传入 rightSibling
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        // TODO(proj2): implement
        int d = metadata.getOrder();
        int threshold = (int) Math.ceil(d * 2 * fillFactor);
        while (data.hasNext()) {
            Pair<DataBox, RecordId> insert = data.next();
            DataBox insertKey = insert.getFirst();
            RecordId insertValue = insert.getSecond();
            int index = InnerNode.numLessThan(insertKey, keys);
            keys.add(index, insertKey);
            rids.add(index, insertValue);
            // 裂开
            if (keys.size() > threshold) {
                List<DataBox> leftKeys = new ArrayList<>(keys.subList(0, threshold));
                List<RecordId> leftRids = new ArrayList<>(rids.subList(0, threshold));
    ...
    public void remove(DataBox key) {
        // TODO(proj2): implement
        // 找到需要删除的节点的索引，然后调用包装好的remove方法即可
        int removeIndex = -1;
    ...
    Optional<LeafNode> getRightSibling() {
        ...
        return Optional.ofNullable(LeafNode.fromBytes(metadata, bufferManager, treeContext, pageNum));
    }
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        return new LeafNode(metadata, bufferManager, page, keys, rids, Optional.of(siblingSize), treeContext);
    }
