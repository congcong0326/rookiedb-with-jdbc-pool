...
public class BPlusTree {
        ...
        if (leafNode != null) {
            ...
            for (int i = 0; i < keys.size(); i++) {
                if (Objects.equals(keys.get(i), key)) {
                    return Optional.ofNullable(leafNode.getRids().get(i));
                }
            }
        }
        // 如果为空，则返回空
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> middleNode = root.put(key, rid);
        middleNode.ifPresent(this::newRoot);
        return;
    }
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        // 需要持续消费数据，直到迭代器达到末尾
        while (data.hasNext()) {
            Optional<Pair<DataBox, Long>> middleNode = root.bulkLoad(data, fillFactor);
            // 节点分裂，溢出到了根节点
            middleNode.ifPresent(this::newRoot);
        }
        return;
    ...
    private void newRoot(Pair<DataBox, Long> middleNode) {
        List<DataBox> keys = new ArrayList<>();
        List<Long> children = new ArrayList<>();
        keys.add(middleNode.getFirst());
        children.add(root.getPage().getPageNum());
        children.add(middleNode.getSecond());
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        public BPlusTreeIterator(LeafNode leafNode, DataBox beginDataBox) {
            this.leafNode = leafNode;
            currentIndex = InnerNode.numLessThan(beginDataBox, leafNode.getKeys());
        }
        ...
        public RecordId next() {
            // TODO(proj2): implement
            RecordId recordId = leafNode.getRids().get(currentIndex++);
            return recordId;
        }
    }
}
