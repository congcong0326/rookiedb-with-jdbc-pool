...
abstract class BPlusNode {
     ...
     *
     * case: 当内部节点分裂时，前 d 个条目保留在左节点，后 d 个条目移至右节点。中间条目将向上移动（而非复制）作为分裂键。
     ...
     * 如果没有发生页分裂，则返回 Optional.empty()
     * B+树总是将数据插入到叶子节点中，如果目标叶子页已经满了（即包含的键数量等于其最大容量），则分裂该叶子页
     * todo 规则需要确认
     * 在该项目中，d 表示 B+ 树的阶，每个节点最大 2d 个键，
     * 叶子节点分裂时：键数量达到 2d + 1，左节点保留d个键，右节点接收d+1个键，裂键取右节点第一个键（示例中的键3）
     * 内部节点分裂时（叶子节点的中间键上升给它的父节点）：键数量 > 2d，左节点保留d个键，右节点保留d个键，分裂键取中间键（示例中的键3）
     */
     ...
     *
     * 优化批量加载时的IO效率，减少节点分裂次数，为后续插入预留空间
     * 普通put在叶子节点填满 2d+1 才分裂，if (keys.size() > 2 * d) split()，内部节点需要持续加载右侧节点？可能优先填充右子树，以减少树的高度变化，保持结构平衡
     * 该方法：当叶子节点填充到 fillFactor * 最大容量 +1 时即分裂，右兄弟仅保留1条记录
     * 示例：当阶数 d=5，fillFactor=0.75 时 最大容量 = 2*5+1 = 11 条 填充阈值 = ceil(11 * 0.75) = 9 条 当节点包含 9+1=10 条时触发分裂
     */
     ...
     * at n.
     *  生成DOT语言格式的树结构片段，用于Graphviz可视化
     *  示例输出：node1 [label="Inner|3|10|20"];
     */
    public abstract String toDot();

    // Serialization ///////////////////////////////////////////////////////////
    /** n.toBytes() serializes n.
     * 将节点数据序列化为字节数组，用于持久化存储
     * */
     ...
     *
     * 从磁盘页加载B+树节点（核心逻辑）：
     * 通过BufferManager获取指定页：Page p = bufferManager.fetchPage(...)
     * 读取首字节判断节点类型：
     * - 1: 叶节点 → LeafNode.fromBytes()
     * - 0: 内部节点 → InnerNode.fromBytes()
     * 最后解除页锁定：p.unpin()
     *
     */
